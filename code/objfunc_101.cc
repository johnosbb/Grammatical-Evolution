				       ...
				       const int id, const int GE_Report_flag)
{
...
#endif
  converter cvrt;
  cvrt.process(genes,n,binarystring);
#ifdef DEBUG_2
  cout << " ";
  ...
  cout << "  ";
#endif
  ge mapobject(&fitness,stringlength,binarystring,1,GE_Report_flag);
  evaluations += 1;
#ifdef DEBUG_1
  cout << "  Length :" << stringlength/8 << "   Fitness:" << fitness << endl;
#endif
  ...
  if(fitness==TARGET_SCORE) 
	...
	cout << genes[f] << " ";
      ...
      save_results_data("testfile",evaluations,genes,n);
      delete [] binarystring;
      exit (1);
      #endif
    }
...
int objective_function::save_results_data(char filename[],int data ,double genes[] , int n)
{
  ...
  save_results_file << "1  " << data << " ";
  for(int f = 0; f < n; f++)
    save_results_file << genes[f] << ",";
  save_results_file << endl;
